...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (!isUserLoading && user && profile) {
      ...
      } else if (profile.userType === 'trainer') {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error instanceof FirebaseError) {
        ...
        if (error.code === 'auth/wrong-password') description = 'Senha incorreta.';
      }
    ...
    >
      <div className="mb-6">
        <Tabs value={loginRole} onValueChange={(v) => setLoginRole(v as any)} className="w-full">
          <TabsList className="grid grid-cols-3 w-full h-12 rounded-2xl bg-muted/20 p-1">
      ...
      <form className="space-y-4" onSubmit={handleLogin}>
        <div className="space-y-2">
          <Label htmlFor="email" className="font-black text-[10px] uppercase text-white/40 tracking-widest">E-mail</Label>
          <Input 
            id="email" 
            type="email" 
            ...
            disabled={isLoading} 
            className="rounded-xl h-12 border-none bg-white/5 text-white font-bold" 
          />
        </div>
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <Label htmlFor="password" className="font-black text-[10px] uppercase text-white/40 tracking-widest">Senha</Label>
            <Link href="/forgot-password" university-style className="text-[10px] font-bold underline text-white/20">Esqueceu?</Link>
          </div>
          <Input 
            id="password" 
            type="password" 
        ...
        <Button 
          type="submit" 
          className="w-full bg-primary hover:bg-primary/90 text-white h-14 rounded-2xl font-black text-lg shadow-xl shadow-primary/20 transition-all active:scale-95 uppercase tracking-tight" 
          disabled={isLoading}
        >
      ...
      <div className="mt-8">
        <div className="relative flex items-center justify-center mb-6">
          ...
          <span className="absolute bg-card px-4 text-[10px] font-black text-white/20 uppercase tracking-widest">Ou continue com</span>
        </div>
        <Button variant="outline" className="w-full rounded-2xl h-12 border-white/10 font-black text-[10px] uppercase tracking-widest hover:bg-white/5 text-white" type="button" disabled={isLoading}>

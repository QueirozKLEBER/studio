    ...
    >
      <div className="mb-6">
        <Tabs value={loginRole} onValueChange={(v) => setLoginRole(v as any)} className="w-full">
          <TabsList className="grid grid-cols-3 w-full h-12 rounded-2xl bg-slate-100 p-1">
      ...
      <form className="space-y-4" onSubmit={handleLogin}>
        <div className="space-y-2">
          <Label htmlFor="email" className="font-black text-[10px] uppercase text-slate-500">E-mail</Label>
          <Input id="email" type="email" placeholder="seu@email.com" required value={email} onChange={(e) => setEmail(e.target.value)} disabled={isLoading} className="rounded-xl h-12 border-slate-200 bg-slate-50/50" />
        </div>
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <Label htmlFor="password" className="font-black text-[10px] uppercase text-slate-500">Senha</Label>
            <Link href="/forgot-password" className="text-[10px] font-bold underline text-slate-400">Esqueceu?</Link>
          </div>
          <Input id="password" type="password" required value={password} onChange={(e) => setPassword(e.target.value)} disabled={isLoading} className="rounded-xl h-12 border-slate-200 bg-slate-50/50" />
        </div>
        <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-white h-14 rounded-2xl font-black text-lg shadow-xl shadow-primary/20 transition-all active:scale-95" disabled={isLoading}>
      ...
      <div className="mt-8">
        <div className="relative flex items-center justify-center mb-6">
          ...
          <span className="absolute bg-white px-4 text-[10px] font-black text-slate-300 uppercase">Ou continue com</span>
        </div>
        <Button variant="outline" className="w-full rounded-2xl h-12 border-slate-200 font-bold hover:bg-slate-50" type="button" disabled={isLoading}>
